...
def created_img_download(img_path, page_count, tabMode, urlMode):
    headers = {
        "user-agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.87 Safari/537.36"}
    page = 1
    folder_status = True
    arcaliveURL_list = ['https://arca.live/b/lastorigin{}category=%EC%B0%BD%EC%9E%91%EB%AC%BC&target=all&keyword=&p={}',
                        'https://arca.live/b/lastorigin{}category=%EC%B0%BD%EC%9E%91%EB%AC%BC(%EC%95%BC%EC%A7%A4)&target=all&keyword=&p={}',
                        'https://arca.live/b/lastorigin{}category=%EC%95%BC%EC%A7%A4&target=all&keyword=&p={}']
    rcmMode = ['?', '/?mode=best&']
    tabMode = tabMode
    urlMode = urlMode
    while True:

        soup = wc(url=arcaliveURL_list[urlMode].format(rcmMode[tabMode], page))
        detail = soup.find_all("a", "vrow")
        base_url = "https://arca.live"
